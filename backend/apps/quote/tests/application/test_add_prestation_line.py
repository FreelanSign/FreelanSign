...
def test_usecase_add_line_ok():
    uc = AddPrestationLineToQuote(
        ...
        prestations=FakePrestations(
            PrestationDTO(id="P1", name="Audit", area_name="SEO", default_rate_cents=10000, weight_days=2, status="active")
        ),
    )
    q, li = uc.execute(
        AddPrestationLineInput(
            quote_id="Q1", prestation_id="P1", owner_vat_exempt=False, owner_default_rate_pct=Decimal("20.00")
        )
    )
