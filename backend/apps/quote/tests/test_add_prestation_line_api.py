...
def test_add_prestation_line(api_client, django_user_model, mock_pdf_and_email):
    ...
    from apps.catalog.models import Prestation, Area  # adapte si besoin

    area = Area.objects.create(name="SEO")
    cli = Client.objects.create(owner=user, name="ACME")
    q = Quote.objects.create(owner=user, client=cli, title="T", reference="REF-ADD",
                              currency="EUR", language="fr", status=Quote.Status.DRAFT, issue_date="2025-01-01",
                              subtotal=Decimal("0"), tax_total=Decimal("0"), discount_total=Decimal("0"), total=Decimal("0"))
